...
if (FALSE){
  eea_rgrid_url <- "https://www.eea.europa.eu/data-and-maps/data/eea-reference-grids-2/gis-files/"
  lapply(countries, function(x){
    download.file(paste(eea_rgrid_url, paste0(tolower(x),"-spatialite"), "at_download", "file", sep = "/"),
                  destfile = file.path(gis_data_path, paste0(x, "_spatialite.zip")),
                  mode = "wb")
    unzip(zipfile = file.path(gis_data_path, paste0(x, "_spatialite.zip")), exdir = gis_data_path)
