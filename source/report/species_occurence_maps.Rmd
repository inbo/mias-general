...
plot_occ_list <- lapply(
  1:length(occ_data_list),
  function(x){
     plot_occ_gbif(
       occ_data_sf = occ_data_list_sflamb[[x]],
       map_data_sf = map_flan,
       plot_title = paste("Occurrences of", occ_data_list[[x]]$hierarchy[[1]] |> dplyr::filter(rank == "species") |> dplyr::pull(name), "in Flanders"),
       plot_subtitle =paste("Total count =", occ_data_list[[x]]$meta$count)
     )
  }
)
```
