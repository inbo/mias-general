...
### Score porportions "unknown" vs. rest per species
...
response_options_data <- get(load((paste0(response_data_path, "recoded_processed/", "results_methods_options.rda"))))
#
#
#
...
#
# get moni_recoded
moni_recoded <- res_open |> # or use open
  dplyr::filter(!question_scored |> as.logical(), !section_skipped) |>
...
# alternatively: just use response_text_final for all options?
response_options_other <- moni_recoded |>
  dplyr::filter(grepl("D8", question_id) & grepl("followup", question_text)) |>
...
response_options_data <- data.frame(
  ...
  dplyr::arrange(response_options_upd)
...
#
# HERE: additional response options lost
#
# get response options per species in long format
...
plot_moni_patt <- ggplot2::ggplot(
  ...
  ggplot2::facet_grid(
    rows = ggplot2::vars(kingdom), #ggplot2::vars(prius_milieu),
    scales = "free",
    space = "free"
  ) +
  ggplot2::geom_point(size = 3) +
...
plot_moni_patt
# HERE
# beverrat monitoring van bever ontbreekt?
# (highlight geen & ik weet het niet, put first, maybe in separate facet col)
# andere recoded?
# remove andere if alternative entry was "waarnemingen.be" etc.
