...
pkg_prima <- extract_pkgs_from_source(
  ...
  dplyr::filter(!grepl("grDevices", pkg))
...
Note that the following information is extracted from the current version (date: `r Sys.Date()`) of the *renv.lock* file in the github repository [inbo/mias-general](https://github.com/inbo/mias-general) [@adolf_monitoring_nodate], which contains all source code associated with this project.
