...
# remove last 6 species for shortlist
species_shortlist_committee <- species_strings_committee |> head(x = _, -6)

# get full species table
species_committee <- table_filtered_upd |>
  dplyr::filter(
    grepl(
      paste(species_strings_committee, collapse = "|"),
      vern_name_nld,
      ignore.case = TRUE
    )
  )
...
table_committee <- species_committee |>
  ...
  dplyr::mutate(
    scenario_1 = dplyr::case_when(
      grepl(vern_name_nld, table_syn_plants_display_list$abundance_management) |
      grepl(vern_name_nld, table_syn_plants_display_list$distribution_management) |
      grepl(vern_name_nld, table_syn_animals_display_list$abundance_management) |
      grepl(vern_name_nld, table_syn_animals_display_list$distribution_management) ~
        "scenario 1"
    ),
    scenario_2 = dplyr::case_when(
      grepl(vern_name_nld, table_syn_plants_display_list$detection) |
      grepl(vern_name_nld, table_syn_animals_display_list$detection) ~
        "scenario 2"
    ),
    scenario_3 = dplyr::case_when(
      grepl(vern_name_nld, table_syn_plants_display_list$distribution)|
      grepl(vern_name_nld, table_syn_animals_display_list$distribution) ~
        "scenario 3"
    )
  ) |>
  dplyr::ungroup() |>
...
table_committee_display <- table_committee |>
  ...
  dplyr::mutate(
    dplyr::across(
      ...
      \(x) {
        dplyr::case_when(
          grepl(
            paste(species_shortlist_committee, collapse = "|"),
            vern_name_nld,
            ignore.case = TRUE
          ) ~
  ...
  kableExtra::column_spec(
    ...
    kableExtra::column_spec(
      column = seq_along(id_cols_display)[-1],
      width = .col_id_width
    ) |>
    kableExtra::column_spec(
      column = 1:2 + length(id_cols_display),
