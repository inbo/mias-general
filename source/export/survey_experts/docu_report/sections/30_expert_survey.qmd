...
For each species, a separate questionnaire was set up using Google Forms. We opted for species-specific forms because we wanted to include species-specific information (e.g., scientific names, distribution maps based on recent data from the Global Biodiversity Information Facility (GBIF) [@gbif_the_global_biodiversity_information_facility_what_2025; @gbiforg_user_occurrence_2024] (@sec-app-maps)). 
...
First, we check whether species-scope combinations can be considered covered by opportunistic observations (e.g. observations recorded via [waarnemingen.be](https://waarnemingen.be/)). If for a species opportunistic observations are considered highly representative by the expert, all scopes retained for this species are marked as having low priority.
