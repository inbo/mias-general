...
tmp <- table_filtered_upd |> 
  dplyr::filter(grepl("plant", kingdom) & grepl("highprior", scope_prior))
...
tmp <- table_filtered_upd |> 
  ...
  tidyr::pivot_wider(
    ...
    values_from = vern_name_nld
    ) |> 
  tidyr::unnest(
    data = _,
    cols = kreeftenvallen # c(`environmental DNA: water`)
    )
