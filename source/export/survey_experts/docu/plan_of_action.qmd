...
make_table_syn_display <- function(
  ...
  colnames_upd <- c(
    ...
    LETTERS[1:(ncol(table_i) - length(cols_id))]
  )
  footnote_i <- c(
    grep("method_", colnames(table_i), value = TRUE) |> gsub("method_", "", x = _)
  )
  table_i |>
    knitr::kable(
      x = _,
    ...
    #  row = 0, 
    #  angle = -90
    #  #extra_css = 'transform-origin: bottom left; text-align: left; padding: 10px'
    #extra_css = 
    #  'transform: rotate(-90deg); height: 200px;'
    #) |>
    kableExtra::row_spec(
      row = 1:nrow(table_i),
      # row height
    ...
    kableExtra::footnote(
      symbol = footnote_i,
      symbol_manual = LETTERS[1:(ncol(table_i) - length(cols_id))]
    ) 
}
...
If a species is covered by one of the methods reported for the respective scope, the table cell is highighted and the total number of species covered by that method is displayed.
...
Examples in the following tables:

- Table for scope **detection**: 
    - We see that **eDNA** (here method 'A') is suited for the detection of 7 species (crayfish and fish) that (also) occur in **freshwater habitats**
    - **Electrofishing and/or trapping** (here methods 'C', 'D', 'E') could be used for putting positive findings to a more specific test

- Table for scope **distribution**:
    - We see that **eDNA** (here method 'B') is suited for determining the distribution of 6 species that (also) occur in **freshwater habitats** (4 crayfish, crab, and fish, 2 mammals)
        - the method could again be backed-up by **electrofishing and/or trapping** (here methods 'D', 'E', 'G') or by **cameratraps** (here method 'C')
    - We also see that **camera traps** could be used for 4 (also) **terrestrial** species

- Table for scope **abundance**:
    - We see that **visual surveys** (here method 'A') are suited to determine the abundance of 3 (also) freshwater species (1 amphibian, 1 crayfish, 1 mammal) 
    - We see that **eDNA** (here method 'B') is suited to determine the abundance of 3 (also) freshwater species (1 amphibian, 1 crab, 1 fish) 
    - We see that **electrofishing** (here method 'C') is suited to determine the abundance of 3 (also) freshwater species (1 amphibian, 1 crayfish, 1 fish) 
    - We see that **trapping** (here method 'D') is suited to determine the abundance of 3 (also) freshwater species (1 amphibian, 1 fish, 1 crab) 
    - We see that **passive acoustic monitoring** (here method 'E') is suited to determine the abundance of 3 (also) terrestrial species (3 mammals) 
    
- Table for scope **abundance after management**:
    - We see that **eDNA** (here method 'A') is suited to determine the abundance after management of 3 (also) freshwater species (1 amphibian, 1 crab, 1 mammal) 
    
It is again possible to use the survey-derived global priority scores to rank order these groups.


```{r synergies-animals-print}
#| results: asis
#| cache: false
#| classes: .center-table
#| eval: true

for (i in scope_type_list |> names()){
  cat(stringr::str_glue("##### Scope: {i}<br>"))
  table_grouped_animals_display_list[[i]] |>
    unclass() |> 
    cat()
  cat("<br>")
}
